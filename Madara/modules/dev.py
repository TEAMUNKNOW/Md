...
from Madara import DEV_USERS, OWNER_ID, dispatcher, telethn
...
def allow_groups(update: Update, context: CallbackContext):
    args = context.args
    if not args:
        state = "off" if Madara.ALLOW_CHATS else "ʟᴏᴄᴋᴅᴏᴡɴ ɪs " + "on"
        update.effective_message.reply_text(f"ᴄᴜʀʀᴇɴᴛ sᴛᴀᴛᴇ: {state}")
        ʀᴇᴛᴜʀɴ
    if args[0].lower() in ["off", "no"]:
        Madara.ALLOW_CHATS = True
    elif args[0].lower() in ["yes", "on"]:
        Madara.ALLOW_CHATS = False
    else:
...
def leave(update: Update, context: CallbackContext):
    ...
    if args := context.args:
        ...
        update.effective_message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup(Madara_leave_bt),
        )
...
def restart(update: Update, context: CallbackContext):
    ...
    process = subprocess.run(
        "pkill python3 && python3 -m Madara", shell=True, check=True
    )
    process.communicate()


@dev_plus
def logss(update: Update, context: CallbackContext):
    user = update.effective_user
    message = update.effective_message
    with open("log.txt", "rb") as f:
        context.bot.send_document(document=f, filename=f.name, chat_id=user.id)
    message.reply_text("ʟᴏɢs sᴇɴᴛ ᴛᴏ ʏᴏᴜʀ ᴅᴍ")
...
LOGSS_HANDLER = CommandHandler("logs", logss, run_async=True)
...
dispatcher.add_handler(LOGSS_HANDLER)
